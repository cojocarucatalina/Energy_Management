...
const UserTable = ({ tableData, onDeleteSuccess, onUpdateClick }) => {
    ...
    const handleDelete = async (user) => {
        try {
            await axios.delete(`http://localhost:8083/user/${user.id}`);
            const encodedEmail = encodeURIComponent(user.email);
            await axios.delete(`http://localhost:8081/device/delete/${encodedEmail}`);
    ...
    const handleSaveChanges = async (e) => {
        e.preventDefault();
        try {
            await axios.put(`http://localhost:8083/user/${currentUser.id}`, currentUser);
            if (originalEmail !== currentUser.email) {
                await axios.put(
                    'http://localhost:8081/device/updateEmail',
                    {
