...
const DevicesPage = () => {
  ...
  const previousConsumptions = useRef({});
    ...
    if (email) {
      const fetchAllData = () => {
        ...
        if (selectedDeviceId && selectedDate) {
          fetchEnergyDataForDevice(selectedDeviceId, selectedDate);
        }
      };
  ...
  }, [email, selectedDeviceId, selectedDate]);

  const fetchDevices = async (email) => {
    try {
      const response = await fetch('http://localhost:8081/device/user-devices', {
        method: 'POST',
        headers: {
  ...
  const fetchEnergyDataForDevice = async (deviceId, date) => {
    try {
      ...
      const response = await fetch(`http://localhost:8082/measurement/from-date?startDate=${formattedDate}&device_id=${deviceId}`);
      if (response.ok) {
        const data = await response.json();
       ...
       const device = Array.isArray(devices) ? devices.find((device) => device.id === deviceId) : null;

       const deviceThreshold = device?.mhec || 0;

        if (totalConsumption > deviceThreshold) {
          const previousTotalConsumption = previousConsumptions.current[deviceId] || 0;

          if (totalConsumption > previousTotalConsumption) {
            setNotificationMessage(
              `Energy consumption for device ${deviceId} has exceeded the threshold (${deviceThreshold})! Total: ${totalConsumption} kWh`
            );
            setIsNotificationVisible(true);

            previousConsumptions.current[deviceId] = totalConsumption;
        ...
        setDeviceCharts((prev) => ({
          ...
          [deviceId]: {
            ...
            datasets: [
              {
                label: `Energy Consumption (kWh) for Device ${device?.description}`,
  ...
  const handleDeleteDevice = async (deviceId) => {
    try {
      const response = await fetch(`http://localhost:8081/device/${deviceId}`, {
        method: 'DELETE',
      });
  ...
  const handleCreateOrUpdateDevice = async (event) => {
    ...
    const url = isEditing
      ? `http://localhost:8081/device/${newDevice.id}`
