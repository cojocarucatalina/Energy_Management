...
//    }
    ...
    private boolean deleteDevicesByUserEmail(String email) {
        try {
            ...
            String url = "http://localhost:8081/device/deleteByUserEmail/" + email;

            restTemplate.delete(url);
            return true;
        } catch (Exception e) {
            LOGGER.error("Failed to delete devices in device service", e);
            return false;
        }
    ...
    public ResponseEntity<Void> deleteUserReferenceByEmail(@PathVariable("email") String email) {
        userReferenceService.deleteByEmail(email);
        return new ResponseEntity<>(HttpStatus.NO_CONTENT);
    }
